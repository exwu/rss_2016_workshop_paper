...
\begin{abstract}
Robotic collaboration requires not just communication from the human to the robot, but also from the robot to the human. \stnote{Needs to get at the idea of speed and accuracy, that the robot needs to respond quickly, that the person is hard to understand.  Social feedback is our solution to a problem so you have to open with the problem.}   We call this robot-to-human communication \emph{social feedback}. In order to flexibly and intelligently generate social feedback, we present a partially Observable Markov Decision Process\stnote{name the model} that incorporates the human's belief about the robot's intent. Doing so incentivizes the robot to communicate its current state \stnote{using language and gesture} in order to allow the human participant to make informed decisions about their next actions. Initial evaluation in simulation shows \stnote{decreased interaction times} \stnote{cut: improvement in interaction length} in a simulated domain while maintaining near-perfect accuracy. 
\end{abstract}

	
\section{Introduction}

\exwnote{add figure with example, 3-5 sentence summary at very end of introduction, extended description in figure caption}
...
\begin{center}
	...
	\end{tikzpicture}
\caption{A graphical model representation of a POMDP \stnote{cut}}
\label{fig:pomdp}
\end{center}
\end{figure}
