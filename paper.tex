...
We will solve this problem by formulating it as a Partially Observable Markov Decision Process (POMDP)\citep{kaelbling99}, which will allow us to dynamically and flexibly determine how to choose social feedback actions. First, we will describe a two agent model and use this to motivate the construction of a POMDP. The crucial improvement we make over existing robot interaction models is that we maintain a state variable that tracks the human's interpretations of the actions the robot takes. 
...
\begin{center}
	\begin{tikzpicture}[node distance = 1.5cm, auto]
		...
		\path [line] (oht) edge (stp1); 
	\end{tikzpicture}
	\caption{A two-agent model based off two POMDPs \stnote{Let's name this model and call it our model.}}
	\label{fig:twoagent}
\end{center}
\end{figure}
